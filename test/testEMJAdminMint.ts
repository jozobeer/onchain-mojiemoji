...
describe("Mint EMJ as admin", () => {
    ...
    it("Other than owner can't adminMint", async () => {
        ...
        await expect(instance.connect(alice).adminMintTo(bob.address, 50)).revertedWith(
    ...
    it("Even admin can't mint over the limit", async () => {
        ...
        await expect(instance.adminMint(10)).to.emit(instance, "Transfer").withArgs(ZeroAddress, deployer.address, 10)
    ...
    it("Can mint again after the limit is increased", async () => {
        ...
        await expect(instance.adminMint(20)).to.emit(instance, "Transfer").withArgs(ZeroAddress, deployer.address, 20)

        expect(await instance.totalSupply()).to.equal(20)

        // check no longer mintable
        await expect(instance.adminMint(1)).to.be.reverted

        await instance.setMintLimit(35)

        await expect(instance.adminMint(15)).to.emit(instance, "Transfer").withArgs(ZeroAddress, deployer.address, 35)

        expect(await instance.totalSupply()).to.equal(35)
    })
    ...
    it("Can't set the limit to less than the last minted tokenId", async () => {
        ...
        await expect(instance.setMintLimit(19)).to.be.revertedWith("mint limit must be greater than the last token ID")
    })
})
